...
def to_dense(data, dataset_info, device=None):
    ...
    data = EquiPlaceHolder(X=X, charges=charges, pos=pos, E=E, y=y,  node_mask=node_mask)
    return data.mask()


class EquiPlaceHolder:
    ...
    def copy(self):
        return EquiPlaceHolder(X=self.X, charges=self.charges, E=self.E, y=self.y, pos=self.pos, t_int=self.t_int, t=self.t,
                           node_mask=self.node_mask)
