...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    ...
    def test_step(self, data, i):
        ...
        self.save_cond_samples(
            samples,
            target_properties,
            file_path=os.path.join(os.getcwd(), f"cond_smiles{i}.pkl"),
        )
    ...
    def test_epoch_end(self, outs) -> None:
        ...
        wandb.log({"final mae": final_mae, "final validity": final_validity})
    ...
    def sample_batch(
        ...
        for s_int in reversed(range(0, self.T)):
            ...
            sampled_s, discrete_sampled_s = self.sample_p_zs_given_zt(
                s_norm, t_norm, X, E, y, node_mask, input_properties
            )
        ...
        if self.visualization_tools is not None:
            ...
            for i in range(num_molecules):
                result_path = os.path.join(
                    current_path,
                    f"chains/{self.args.general.name}/"
    ...
    def cond_sample_metric(self, samples, input_properties):
        ...
        for sample in samples:
            ...
            if self.args.general.guidance_target in ["mu", "both"]:
                ...
                dipole_moment = (
                    math.sqrt(dip_x**2 + dip_y**2 + dip_z**2) * 2.5417464519
                )
        ...
        if self.args.general.guidance_target == "mu":
            ...
            mae = self.cond_val(
                mols_homo.unsqueeze(1), input_properties.repeat(len(mols_homo), 1).cpu()
            )
            ...
            mae = self.cond_val(
                properties, input_properties.repeat(len(mols_dipoles), 1).cpu()
            )
    ...
    def sample_p_zs_given_zt(self, s, t, X_t, E_t, y_t, node_mask, input_properties):
        ...
        prob_X = prob_X_unnormalized / torch.sum(
            prob_X_unnormalized, dim=-1, keepdim=True
        )

        prob_E_unnormalized = p_eta_e * prob_E
        prob_E_unnormalized[torch.sum(prob_E_unnormalized, dim=-1) == 0] = 1e-7
        prob_E = prob_E_unnormalized / torch.sum(
            prob_E_unnormalized, dim=-1, keepdim=True
        )
