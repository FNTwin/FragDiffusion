...
class Qm9RegressorDiscrete(pl.LightningModule):
    ...
    def configure_optimizers(self):
        optimizer = torch.optim.AdamW(
            self.parameters(), lr=self.args.train.lr, amsgrad=True, weight_decay=1e-12
        )
    ...
    def on_train_epoch_end(self) -> None:
        ...
        print(
            f"Epoch {self.current_epoch}: train_mse: {train_mse :.3f} -- {time.time() - self.start_epoch_time:.1f}s "
        )
    ...
    def compute_extra_data(self, noisy_data):
        ...
        assert (
            extra_X.shape[-1] == 0
        ), "The regressor model should not be used with extra features"
        assert (
            extra_E.shape[-1] == 0
        ), "The regressor model should not be used with extra features"
