...
def get_eigenvectors_features(vectors, node_mask, n_connected, k=2):
    ...
    random_mask = (50 * torch.ones((bs, n, k0)).type_as(vectors)) * (
        ~node_mask.unsqueeze(-1)
    )  # bs, n, k0
    min_batched = torch.min(quantized + random_mask, dim=1).values.unsqueeze(
        1
    )  # bs, 1, k0
    max_batched = torch.max(quantized - random_mask, dim=1).values.unsqueeze(
        1
    )  # bs, 1, k0
    nonzero_mask = quantized.abs() >= 1e-5
    is_min = (
        (quantized == min_batched) * nonzero_mask * node_mask.unsqueeze(2)
    )  # bs, n, k0
    is_max = (
        (quantized == max_batched) * nonzero_mask * node_mask.unsqueeze(2)
    )  # bs, n, k0
    is_other = (
        (quantized != min_batched)
        * (quantized != max_batched)
        * nonzero_mask
        * node_mask.unsqueeze(2)
    )

    all_masks = torch.cat(
        (is_min.unsqueeze(-1), is_max.unsqueeze(-1), is_other.unsqueeze(-1)), dim=3
    )  # bs, n, k0, 3
